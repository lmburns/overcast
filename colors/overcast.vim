...
" License: MIT                                                  "
" Source: http://github.com/lmburns/overcast                      "
" ===============================================================
...
let s:overcast_version = '0.1'

if !(has('termguicolors') && &termguicolors) && !has('gui_running') && &t_Co != 256
  finish
endif

let s:overcast_transparent_background = get(g:,'overcast_transparent_background', 0)
let s:overcast_background = get(g:,'overcast_background','darkest')
let s:overcast_allow_bold = get(g:,'overcast_allow_bold', 0)
let s:overcast_allow_italic = get(g:,'overcast_allow_italic', 0)
let s:overcast_allow_underline = get(g:,'overcast_allow_underline', 0)
let s:overcast_allow_reverse = get(g:,'overcast_allow_reverse', 0)
let s:overcast_allow_undercurl = get(g:,'overcast_allow_undercurl', 0)

let s:bold = ''
if s:overcast_allow_bold == 1
  let s:bold = ' cterm=bold gui=bold '
endif

let s:italic = ''
if s:overcast_allow_italic == 1
  let s:italic = ' cterm=italic gui=italic '
endif

let s:underline = ''
if s:overcast_allow_underline == 1
  let s:underline = ' cterm=underline gui=underline '
endif

let s:reverse = ''
if s:overcast_allow_reverse == 1
  let s:reverse = ' cterm=reverse gui=reverse '
endif

let s:undercurl = ''
if s:overcast_allow_undercurl == 1
...
let s:colors = {
      \ 'bg0':             s:background[s:overcast_background],
      \ 'bg1':             ['#333b41',   '237'],
      \ 'bg2':             ['#3a4248',   '235'],
      \ 'bg3':             ['#3a4248',   '237'],
      \ 'bg4':             ['#445055',   '237'],
...
function! g:OvercastPalette()
  return copy(s:colors)
endfunction

function! g:Overcast()
  echo s:overcast_version
endfunction
...
function! s:apply_syntax_highlightings()
  if s:overcast_transparent_background
...
" }}}
  ...
  exec 'hi rLstElmnt' . s:fg_orange
  exec 'hi rDollar' . s:fg_green . s:bold
  "===============================================================
  " Matlab:
  "===============================================================
...
command! -nargs=0 Overcast :call g:Overcast()
